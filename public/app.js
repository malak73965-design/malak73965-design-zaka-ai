...
function renderHistory() {
  ...
  if (!state.chats.length) {
    box.innerHTML =
      '<div class="history-empty">لا توجد محادثات محفوظة بعد.</div>';
    return;
  }
  ...
  if (!state.messages.length) {
    box.innerHTML = `
      <div class="empty-state" id="emptyState">
        ...
        <h2>جاهز لك</h2>
        <p>
          اختر النموذج واكتب رسالتك.
          النماذج المتاحة تُكتشف تلقائيًا من API.
        </p>
      </div>
    `;
...
function addMessageBubble(message, scroll = true) {
  ...
  if (message.role !== 'user') {
    ...
    if (state.config?.providers?.telegram) {
      ...
      sendTelegram.className =
        'msg-action';

      sendTelegram.textContent =
        '✈️ Telegram';
...
async function generateImage(event) {
  ...
  if (result) {
    result.innerHTML =
      '<div class="image-loading">⏳ جاري إنشاء الصورة...</div>';
  }
...
function renderImageResult(result, url, model) {
  ...
  toast(
    `تم إنشاء الصورة عبر ${imageLabel(model)}.`,
    'success'
  );
}
...
async function pollImageJob(
  ...
  if (attempts > 90) {
    throw new Error(
      'استغرق توليد الصورة وقتًا طويلًا.'
    );
  }
  ...
  if (
    ...
    if (!mediaUrl) {
      throw new Error(
        'اكتملت المهمة ولكن لم يصل رابط الصورة.'
      );
    }
...
sendMessage();
  ...
  $('#imageProvider')?.addEventListener(
    'change',
    () => {
      state.imageProvider =
        $('#imageProvider').value;
